...
/** ===========================================================================
 * 頂点には普通、頂点属性と呼ばれる「それぞれの頂点に固有の属性値」があります。
 * 少なくとも頂点の座標は大抵の場合必要になりますが、その他にも頂点自身が色とい
 * う属性持っているようにすることで、頂点ごとに個別に色を着けることができます。
 * ここでは、５つの頂点がそれぞれ別の頂点カラーを持つように拡張してみます。
 * ========================================================================= */
...
class WebGLFrame {
        ...
        return new Promise((resolve) => {
            ...
            .then((shaders) => {
                ...
                this.attLocation = [
                    ...
                    gl.getAttribLocation(this.program, 'color'), // 頂点属性にカラーを追加 @@@
                ];
                this.attStride = [
                    3,
                    4, // ストライドは vec4 用なので 4 を指定 @@@
                ];
                // uniform 変数関係
                this.uniLocation = [
    ...
    setup(){
        ...
        // 頂点カラーの定義 @@@
        this.color = [
            1.0, 1.0, 1.0, 1.0, // １つ目の頂点の色
            1.0, 0.0, 0.0, 1.0, // ２つ目の頂点の色
            0.0, 1.0, 0.0, 1.0, // ３つ目の頂点の色
            0.0, 0.0, 1.0, 1.0, // ４つ目の頂点の色
            0.5, 0.5, 0.5, 1.0, // ５つ目の頂点の色
        ];
        // 定義した頂点の情報（頂点属性）は VBO に変換しておく
        this.vbo = [
            this.createVbo(this.position),
            this.createVbo(this.color), // ← 頂点カラーも VBO に追加！ @@@
        ];
